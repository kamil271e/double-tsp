...
Local search from various random or randomized starting points. You can create
random solutions, or use the randomized heuristics from previous classes.
...
auto TSP::multiple_local_search()
...
/*
Iterative local search with little perturbation. Perturbation1 (ILS1) can
involve, for example, replacing several edges and/or vertices with others
selected at random. The stop condition for ILSx is to reach a time equal to the
average MSLS time for the same instance.

Attention , one run of MSLS includes 100 iterations of LS, and the the final
result is the best solution obtained in those 100 runs. The starting solution
can be a random solution or one obtained using a randomized heuristic.
...
auto TSP::iterative_local_search_one()
    -> std::tuple<std::vector<int>, std::vector<int>> {
  ...
  while (std::chrono::steady_clock::now() < avg_time) {
    ...
    if (calculate_objective(cycle_y1, cycle_y2) <
        calculate_objective(cycle_x1, cycle_x2)) {
      cycle_x1 = cycle_y1;
      cycle_x2 = cycle_y2;
    }
  }
...
auto TSP::perturbation_one(std::vector<int> &c1, std::vector<int> &c2)
    -> std::tuple<std::vector<int>, std::vector<int>> {
  ...
  std::uniform_int_distribution<int> vertex_dist(0, dist_matrix.x_coord.size() -
                                                        1);
  ...
  // Replace the selected vertices with random vertices with the random movement
  // type
  for (int i = 0; i < vertices.size(); ++i) {
    ...
    auto &num_cycle_i =
        std::find(c1.begin(), c1.end(), vertices[i]) != c1.end() ? c1 : c2;
    auto &num_cycle_j =
        std::find(c1.begin(), c1.end(), j) != c1.end() ? c1 : c2;

    // Find index of selected vertex
    int idx_i = std::find(num_cycle_i.begin(), num_cycle_i.end(), vertices[i]) -
                num_cycle_i.begin();
    int idx_j = std::find(num_cycle_j.begin(), num_cycle_j.end(), j) -
                num_cycle_j.begin();

    if (vertices[i] != j) {
      // Check if the selected vertex is in the same num_cycle_j
      // if (std::find(num_cycle_j.begin(), num_cycle_j.end(), j) !=
      // num_cycle_j.end() && std::find(num_cycle_j.begin(), num_cycle_j.end(),
      // vertices[i]) != num_cycle_j.end()) {
      if (num_cycle_i == num_cycle_j) {
        // Update the num_cycle_j (inner class)
        if (movement_type == 0) { // edge
          std::reverse(num_cycle_i.begin() + idx_i,
                       num_cycle_i.begin() + idx_j + 1);
        } else { // vertex
...
/*
Iterative local search with Large-scale neighborhood search, i.e., a larger
Destroy-Repair perturbation.

Perturbation2 (ILS2) should involve removing more edges and vertices (e.g., 30%)
(destroy) and fixing the solution using a heuristic method, one of those
implemented in the first lesson. The vertices/edges to be removed can be chosen
randomly or heuristically, such as those that are close to the second cycle. We
also test this version without a local search (only subject the initial solution
to a local search, as long as the starting solution was random) The stop
condition for ILSx is to reach a time equal to the average MSLS time for the
same instance.
...
auto TSP::iterative_local_search_two()
    -> std::tuple<std::vector<int>, std::vector<int>> {
  std::vector<int> cycle_x1, cycle_x2;
  std::vector<int> cycle_y1, cycle_y2;
...
auto TSP::destroy_perturbation(std::vector<int> &c1, std::vector<int> &c2)
    -> std::tuple<std::vector<int>, std::vector<int>> {
  float coef = 0.5;
  std::random_device rd;
  std::mt19937 gen(rd());
