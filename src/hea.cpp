...
auto TSP::hybrid_evolution_algo() -> std::tuple<std::vector<int>, std::vector<int>>
{
   // Initialization 
...
auto TSP::select_two_parents(const std::vector<std::tuple<std::vector<int>, std::vector<int>>> population) -> std::pair<int, int>
{
...
auto TSP::recombine(const std::vector<int>& parent1, const std::vector<int>& parent2, std::vector<int>& second_cycle) -> std::vector<int>
{
   ...
   auto repairedCycles = find_greedy_cycles_regret_from_incomplete(child1, second_cycle);
...
std::unordered_set<std::pair<int, int>, pair_hash> TSP::findEdges(const std::vector<int>& cycle) {
...
void TSP::remove_edges(std::vector<int>& cycle, const std::unordered_set<std::pair<int, int>, pair_hash>& other_edges) {
    for (auto it = cycle.begin(); it != cycle.end();) {
        int from = *it;
        int to = *(std::next(it) != cycle.end() ? std::next(it) : cycle.begin());
...
void TSP::remove_isolated_vertices(std::vector<int>& cycle, const std::unordered_set<int>& vertices) {
    for (auto it = cycle.begin(); it != cycle.end();) {
