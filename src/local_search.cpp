...
auto TSP::generate_all_vertex_movements(int n)
  ...
  for (int i = 0; i < n; ++i) {
    for (int j = i + 1; j < n; ++j) {
      if (i == 0 && j == n - 1)
        continue; // redundant - edge move (1, n-2) is the same as vertex move
                  // (0, n-1)
      for (int k = 0; k < 2; ++k) {
        movements.push_back({i, j, 0, 0, k});
      }
  ...
  do {
    ...
    for (int iter = 0; iter < movements.size(); ++iter) {
      std::tie(objective_value, cycle_num) =
          get_delta(movements[iter]); // we need to remember chosen cycle for
                                      // apply_movement method; type = 0 -
                                      // cycle1, 1 - cycle2
      if (objective_value > 0) {      // better than current
        //                std::cout << movements[iter][0] << " " <<
        //                movements[iter][1] << " " << movements[iter][2] << " "
        //                << cycle_num << std::endl;
        found_better = true;
        if (steepest) {
